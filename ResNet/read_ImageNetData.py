...
def ImageNetData(data_dir,batch_size,num_workers):
# data_transform, pay attention that the input of Normalize() is Tensor and the input of RandomResizedCrop() or RandomHorizontalFlip() is PIL Image
    ...
    image_datasets['train'] = myImageDataSet(os.path.join(data_dir, 'train'),
                                                   os.path.join(data_dir, 'ILSVRC2012_devkit_t12', 'data', 'meta.mat'),
                                                   data_transforms['train'])
    image_datasets['val'] = myImageDataSet(os.path.join(data_dir, 'val'),
                                               os.path.join(data_dir, 'ILSVRC2012_devkit_t12', 'data',
            ...
            for root, _, fnames in sorted(os.walk(d)):
                for fname in sorted(fnames):
                    if self._is_image_file(fname):
                        ...
                        item = (path, class_to_idx[target.replace(" ", "")])
                        images.append(item)
